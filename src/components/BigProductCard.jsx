  ...
  return (
    <Link to={`${shoe.id}`} className="border border-[1px solid #E5E7EB] rounded-t-[10px]">
      ...
      <div className="pl-2 py-1">
        <h1 className="text-[1.35rem] font-playfair font-medium text-[#000] leading-tight mt-2">{shoe.productName}</h1>
        <p className="text-[1rem] text-[#707072] mt-2">{shoe.category}</p>
        <span className="text-[1.125rem] font-medium block mt-5">{shoe.price}</span>
      </div>
    </Link>
  )
}
